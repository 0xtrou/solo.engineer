...
import { homeSourceDomains } from "@/lib/source-domains";
import {
  defaultFeedFilters,
  parseFeedFilters,
  policySourceIds,
...
function SourceButton({ source, active, quality, onClick }: { source: SourceId; active: boolean; quality?: { composite: number; yieldPct: number; citations: number; total: number }; onClick: () => void }) {
  const composite = quality?.composite ?? 0;
  const tooltip = quality ? `Q${composite} · ${Math.round(quality.yieldPct)}% yield · ${quality.citations} cites · ${quality.total} items` : source;
  return (
    <button
    ...
    >
      ...
      <span className="text-[#d76346]">{composite}</span>
    </button>
  );
}
...
export function FeedDashboard({ initialFeed }: FeedDashboardProps) {
  ...
  const sourceQuality = useMemo(() => {
    ...
    const citations = citationInDegree(items, homeSourceDomains, (i) => i.source);
    return computeSourceQuality(items, {
      sourceKey: (i) => i.source,
      tierFor: (sourceId) => sourceMeta[sourceId as SourceId]?.tier,
  ...
  const byCompositeThenTier = useCallback((a: SourceId, b: SourceId) => {
    const compositeDiff = (sourceQuality.get(b)?.composite ?? 0) - (sourceQuality.get(a)?.composite ?? 0);
    if (compositeDiff !== 0) return compositeDiff;
    return tierIndexOf(a) - tierIndexOf(b);
  }, [sourceQuality, tierIndexOf]);
  const sortedResearchSources = useMemo(() => [...researchSourceIds].sort(byCompositeThenTier), [byCompositeThenTier]);
  const sortedPolicySources = useMemo(() => [...policySourceIds].sort(byCompositeThenTier), [byCompositeThenTier]);
      ...
      <div className="mx-auto grid min-h-screen max-w-[1480px] grid-cols-1 lg:items-start lg:grid-cols-[236px_minmax(570px,1fr)] xl:grid-cols-[236px_minmax(570px,1fr)_340px]">
        <aside className="hidden min-h-screen border-r border-[#e4e6e1] bg-[#fbfcfa] px-[18px] py-7 lg:sticky lg:top-0 lg:flex lg:h-screen lg:overflow-y-auto lg:overscroll-contain lg:flex-col" data-testid="feed-left-sidebar">
          ...
          <nav className="mt-2 space-y-1" aria-label="Research sources">
            ...
            {sortedResearchSources.map((source) => (
              <SourceButton key={source} source={source} active={filters.source === source} quality={sourceQuality.get(source)} onClick={() => setSource(source)} />
            ))}
          </nav>
        ...
        <section className="min-w-0 bg-[#fffdfb] px-5 sm:px-9 lg:px-11">
          ...
          <nav className="tab-scroll -mx-5 flex gap-2 overflow-x-auto border-b border-[#e6e6e0] px-5 py-3 lg:hidden" aria-label="Filter feed by source">
            ...
            {[...mobileSourceIds].sort(byCompositeThenTier).map((source) => (
        ...
        <aside className="hidden bg-[#f8f9f6] px-6 py-7 xl:sticky xl:top-0 xl:block xl:h-screen xl:overflow-y-auto xl:overscroll-contain" data-testid="feed-right-sidebar">
          ...
          <section className="mt-7 border-t border-[#e4e6e1] pt-5">
            ...
            <div className="mt-3 space-y-1.5">
              {feed?.statuses.slice().sort((a, b) => byCompositeThenTier(a.source, b.source)).map((status) => { const q = sourceQuality.get(status.source); const composite = q?.composite ?? 0; return (<div key={status.source} className="flex items-center gap-2 rounded px-1 py-1 text-[11px] text-[#59615c]"><span className={`h-1.5 w-1.5 rounded-full ${status.loaded ? "bg-[#67aa72]" : "bg-[#d99157]"}`} /><span className="flex-1">{sourceMeta[status.source].label}</span><span className="font-mono text-[9px] text-[#d76346]">{composite}</span><span className="font-mono text-[9px] text-[#a1a7a2]">{status.loaded ? "LIVE" : "WAIT"}</span></div>); })}
            </div>
          </section>
